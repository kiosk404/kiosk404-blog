...
### 重传机制
TCP（Transmission Control Protocol，传输控制协议）作为一种可靠的传输协议，其核心任务之一就是确保数据能够可靠、有序地从发送方传输到接收方。在不可靠的网络环境中，数据包可能会丢失、延迟或乱序。**超时机制** 是 TCP 实现可靠传输的关键一环，它与重传机制紧密配合
...
![tcp_net_1](https://img1.kiosk007.top/static/images/blog/20250628122240-net_tcp_1.png)



#### 快速重传

好的，TCP 快速重传（Fast Retransmit）是 TCP 为了提高传输效率而引入的一种**拥塞控制机制的补充**，它旨在**在不等待重传超时（RTO）的情况下，尽快地发现和重传丢失的数据段**。

TCP 还有另外一种**快速重传（Fast Retransmit）机制**，它**不以时间为驱动，而是以数据驱动重传**。

##### 快速重传的原理

快速重传的核心思想是利用**冗余的 ACK（Duplicate ACK）**来推断数据包丢失。

1. **ACK 机制：** TCP 接收方通过发送 ACK 报文来确认已经收到的数据。ACK 报文中的确认号（Acknowledgment Number）表示接收方**期望收到的下一个字节的序号**。
2. **数据包丢失：** 假设发送方发送了数据包 1, 2, 3, 4, 5。如果数据包 3 丢失了。
3. **重复 ACK 的产生：**
   - 接收方收到了数据包 1 和 2，会发送对数据包 3 的 ACK（确认号为 3）。
   - 接收方接着收到了数据包 4。因为数据包 3 还没收到，所以数据包 4 是乱序到达的。此时，接收方仍然会发送对数据包 3 的 ACK（确认号仍为 3），表示它仍然在等待数据包 3。
   - 接收方又收到了数据包 5。数据包 5 也是乱序到达的。接收方会再次发送对数据包 3 的 ACK（确认号仍为 3）。
4. **触发快速重传：** 当发送方连续收到**三个或更多**个**重复的 ACK**（即确认号相同的 ACK）时，发送方就会认为**确认号所指示的数据包（在这个例子中是数据包 3）很可能已经丢失**，而无需等待重传定时器（RTO）超时。
5. **执行快速重传：** 收到三个重复 ACK 后，发送方立即重传那个被重复确认的数据包（数据包 3）。
...
#### TCP 的选择性确认（Selective Acknowledgment，SACK）机制



SACK 是对传统 TCP 确认机制的一种重要扩展，旨在**提高 TCP 在多包丢失环境下的传输效率**。它通过允许接收方精确地告知发送方哪些数据段已经收到（即使这些数据段是不连续的），从而避免了发送方不必要的重传。



#### 1. 传统 ACK 机制（累计确认 - Cumulative Acknowledgment）的局限性



在 SACK 出现之前，TCP 采用的是**累计确认**（Cumulative Acknowledgment）机制。这意味着 ACK 报文中的确认号（Acknowledgement Number）表示接收方**期望收到的下一个字节的序号**，它隐式地确认了所有该序号之前的数据都已经成功收到。

**这种机制在单个或少量数据包丢失时表现良好，但在多包丢失或乱序严重时效率低下：**

- **盲目重传：** 假设发送方发送了数据包 1, 2, 3, 4, 5。如果数据包 2 和 4 丢失了，而数据包 1, 3, 5 都成功到达接收方。
  - 接收方会持续发送对数据包 2 的 ACK（确认号为 2），因为数据包 2 是它期望收到的第一个丢失的数据。
  - 发送方收到重复的 ACK 后，只能知道数据包 2 丢失了，但它**不知道**数据包 3 和 5 其实已经成功到达了。
  - 在重传时，发送方可能不得不从数据包 2 开始，重新发送 2, 3, 4, 5（或直到拥塞窗口允许的最大量），导致**不必要的重传**那些已经到达的数据包（如数据包 3 和 5），浪费带宽。
- **恢复时间长：** 发送方需要依赖重传定时器（RTO）或多次重复 ACK 来逐步发现所有丢失的数据包，恢复过程较慢。
...
SACK 通过在 TCP 选项中添加额外的信息，解决了传统累计确认的盲目性问题。

- **SACK 选项：** 当接收方发现收到的数据段是不连续的（即中间有丢失），并且它支持 SACK 选项时，它会在 ACK 报文中携带 SACK 选项。
- **SACK Block：** SACK 选项中包含一个或多个 **SACK Block**。每个 SACK Block 都由一对数据段的起始序号和结束序号组成，表示接收方已经成功收到的**不连续**的数据段范围。
  - 例如，如果接收方收到了 1-1000 字节，以及 2001-3000 字节，但 1001-2000 字节丢失了。那么 ACK 报文的确认号仍然是 1001（期望收到的下一个字节），但 SACK 选项会包含一个 SACK Block：`[2001, 3001)`，表示它已经收到了从 2001 字节开始到 3001 字节之前的数据。
- **发送方的行为：** 当发送方收到带有 SACK 选项的 ACK 报文时，它会：
  1. 根据 ACK 确认号得知已经收到的连续数据。
  2. 解析 SACK 选项，得知哪些乱序的数据段也已经收到。
  3. 结合这些信息，发送方可以精确地判断出哪些数据段是**真正丢失**的，然后**只重传这些丢失的数据段**，而不会重传那些虽然乱序但已经到达的数据。

```bash
发送方                                                 接收方
--------------                                         --------------
Send: 1 (ACK=1)                                        Receive 1
Send: 2                                                Send ACK=2
Send: 3 (lost)
Send: 4                                                Receive 4
                                                       Send ACK=2, SACK=[4-END_OF_4]  <-- 收到乱序的4，再次发送期望2的ACK，并SACK告知收到4
Send: 5                                                Receive 5
                                                       Send ACK=2, SACK=[4-END_OF_4], [5-END_OF_5] <-- 收到乱序的5，再次发送期望2的ACK，并SACK告知收到4和5

--------------                                         --------------
发送方根据 ACK=2 和 SACK 信息：
  - 知道 1 已收到 (通过 ACK=2)
  - 知道 2 丢失 (ACK=2 期望，但没收到ACK=3)
  - 知道 4 和 5 已收到 (通过 SACK)
  - 结论：真正丢失的是 2 和 3。
  -   只重传数据包 2 和 3。
```
