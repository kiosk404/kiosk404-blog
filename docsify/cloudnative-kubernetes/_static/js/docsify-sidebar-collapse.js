...
(function (global, factory) {
    ...
    // Add 
    var openStatus = JSON.parse(localStorage.getItem('openStatus')) || [];
    ...
    function sidebarCollapsePlugin(hook, vm) {
      hook.doneEach(function (html, next) {
        ...
        // Add
        for (let i = 0; i < openStatus.length; i++) {
          $('.sidebar-nav li.folder').eq(openStatus[i])[0]?.classList.add('open');
        }
        
        // Add
        $('.sidebar-nav a').click(sidebarAClick);
    ...
    //Add
    var lastATop = localStorage.getItem('lastATop')
    function sidebarAClick(e) {
      lastATop = document.querySelector('.sidebar').scrollTop;
      localStorage.setItem('lastATop', lastATop)
    }
  
    function syncAcrollTop() {
      console.log('syncAcrollTop', lastATop)
      document.querySelector('.sidebar').scrollBy(0, lastATop);
    }
    ...
    function handleMenuClick(e) {
      ...
      if (newActiveNode.classList.contains('open')) {
        newActiveNode.classList.remove('open'); // docsify 默认行为会操作 collapse，我们异步之后修补
         ...
         // Add
         var newActiveNodeIndex = $('.sidebar-nav li.folder').index(newActiveNode);
         let _index = openStatus.indexOf(newActiveNodeIndex);
         if ( _index != -1) {
           openStatus.splice(_index, 1);
           localStorage.setItem('openStatus', JSON.stringify(openStatus));
         }
        ...
        // Add
        var newActiveNodeIndex = $('.sidebar-nav li.folder').index(newActiveNode);
        if (newActiveNodeIndex != -1 && openStatus.indexOf(newActiveNodeIndex) == -1) {
          openStatus.push(newActiveNodeIndex);
          localStorage.setItem('openStatus', JSON.stringify(openStatus));
        }
