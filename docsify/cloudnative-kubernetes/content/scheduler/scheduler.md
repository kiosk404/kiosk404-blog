...
[调度算法](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/schedule_algorithm)只对调度周期从PreFilter到Score扩展点进行抽象和实现，那么调度算法又是如何与其他扩展点配合实现Pod的调度呢？这就是本文重点解析的内容，kube-scheduler定义了Scheduler类型，它实现了调度Pod的全流程。
...
Scheduler要想实现调度一个Pod的全流程，那么必须有[调度队列](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/scheduling_queue)、[调度缓存](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/cache)、[调度框架](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/framework)、[调度插件](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/plugin)、[调度算法](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/schedule_algorithm)等模块的支持。所以，Scheduler的成员变量势必会包含这些类型，源码链接：https://github.com/kubernetes/kubernetes/blob/release-1.21/pkg/scheduler/scheduler.go#L62
...
前面笔者提到了recordSchedulingFailure()至少包含记录调度失败事件的功能，还应该有什么功能？有没有发现注入的Scheduler.Error()函数还没有用上？Pod从[调度队列](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/SchedulingQueue.md)弹出Pod如果调度失败是不是应该放回[调度队列](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/SchedulingQueue.md)？这部分实现笔者断定会放在recordSchedulingFailure()函数中，源码链接：https://github.com/kubernetes/kubernetes/blob/release-1.21/pkg/scheduler/scheduler.go#L322
...
现在应该明白为啥需要注入Error函数了，因为调度错误的处理并不是简单的放回到[调度队列](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/scheduling_queue)，期间还要根据Pod最新的状态再做决定。而最新的Pod状态存储在SharedIndexInformer的缓存中，而Scheduler没有SharedIndexInformer成员变量，注入函数的方式完美的解决了这个问题。

### assume



注入NextPod()是因为从[调度队列](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/scheduling_queue)中弹出Pod时写日志，难道单独封装assume()函数也是这个目的？源码链接：https://github.com/kubernetes/kubernetes/blob/release-1.21/pkg/scheduler/scheduler.go#L330
...
1. Scheduler从调度队列中弹出Pod，通过[调度算法](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/schedule_algorithm)选出最优的Node；
2. 如果无Node能够满足Pod资源，则通过PostPlugin实现抢占；
3. 如果选出了最优Node，则经过 ReservePlugin 和PermitPlugin 后Pod进入绑定周期；
