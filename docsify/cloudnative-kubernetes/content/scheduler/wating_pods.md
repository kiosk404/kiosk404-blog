...
本文的等待Pod不是[调度队列](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/scheduling_queue)中等待调度的Pod，在调度队列中的Pod类型是PodInfo。本文的等待Pod是在[调度插件](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/plugin)中PermitPlugin返回等待的Pod，虽然当前没有PermitPlugin的实现，但这不影响我们理解kube-scheduler如何处理多个插件同时返回等待的情况。毕竟有任何插件返回拒绝，Pod就会返回调度队列，全部返回批准，Pod就会进入绑定周期，所以kube-scheduler需要管理至少一个PermitPlugin返回等待并且其他PermitPlugin都返回批准的Pod
