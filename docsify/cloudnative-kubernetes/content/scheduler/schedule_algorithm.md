...
[调度框架](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Framework.md)和[调度插件](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/plugin)将调度流程抽象为若干的阶段，每个阶段称之为一个扩展点，也就是用户可以在每个阶段扩展自定义的 调度插件。由于 调度框架 和 调度插件 相对比较复杂，如果将调度视为一种接口(interface)，那么输入应该是一个Pod和全量的Node，输出是为Pod分配的节点。至于接口的使用者就可以弱化[调度框架](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/framework)和[调度插件](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/plugin)的概念，这是一种友好且通用的抽象。

再者，Pod.Spec.SchedulerName可以选择调度框架来调度该Pod，虽然不同的Pod的调度框架有所不同，但是调用调度插件的流程大部分都是一致的，但是在 调度框架 文章中并没有看到有相关的成员函数的实现。那么，将所有调度插件串接起来，形成最终的调度能力是由随实现的呢？
...
如果对[调度框架](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Framework.md)和[调度插件](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Plugin.md)比较熟悉的话，那么上面的代码就非常好理解了。问：调度一个Pod总共分几步？答：分4步。
...
接下来，笔者对实现以上后三步的代码做详细的解析，因为第一步已经在[调度缓存](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Cache.md)中解析过了。

### 过滤



笔者在[调度插件](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Plugin.md)的文章中提到了，过滤分为[过滤前处理](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Plugin.md#PreFilterPlugin)、[过滤处理](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Plugin.md#FilterPlugin)和[过滤后处理](https://github.com/jindezgm/k8s-src-analysis/blob/master/kube-scheduler/Plugin.md#PostFilterPlugin)，那他们之间是什么样的逻辑关系形成最终的过滤功能？源码链接：https://github.com/kubernetes/kubernetes/blob/release-1.21/pkg/scheduler/core/generic_scheduler.go#L223
...
4. 调度算法并不是对[调度框架](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/framework)和[调度插件](https://kiosk007.top/docsify-book/cloudnative-kubernetes/#/content/scheduler/plugin)再抽象和封装，只是对调度周期从PreFilter到Score的过程的一种抽象，其中PostFilter不在调度算法抽象范围内。因为PostFilter与过滤无关，是用来实现抢占的扩展点；