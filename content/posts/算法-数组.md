...
// getEnd 找到从 start 开始的第 k 个节点
func getEnd(start *ListNode, k int) *ListNode {
    for start != nil {
        k--
        if k == 0 {
            return start
...
// reverse 反转从 a 到 b 之间的节点
func reverse(a, b *ListNode) {
    var pre *ListNode
    cur := a
    for cur != b.Next {
        next := cur.Next
